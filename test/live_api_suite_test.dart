...
void main() {
  ...
  group('OGAGLOW - Live Backend API Integration Suite', () {
    test(
      '1. GET /OGAGLOW/products returns products list and parses correctly',
      () async {
    ...
    test(
      '2. GET /OGAGLOW/products/:id returns specific product details',
      () async {
    ...
    test(
      '3. GET /OGAGLOW/brand/brand-features returns valid brand data',
      () async {
    ...
    test('4. GET /OGAGLOW/about-us returns valid about us model', () async {
      ...
      expect(res.statusCode, 200);
      expect(res.data, isA<Map<String, dynamic>>());
      final aboutModel = AboutUsModel.fromJson(
    ...
    test(
      '6. POST /OGAGLOW/orders/checkout/preview calculates price breakdown',
      () async {
        expect(sampleProductId, isNotNull);
        final payload = {
          'orderItems': [
        ...
        final res = await dio.post(
          '/OGAGLOW/orders/checkout/preview',
          data: payload,
        );
        expect(res.statusCode, 200);
        final preview = CheckoutPreviewResponse.fromJson(
          res.data as Map<String, dynamic>,
        );
    ...
    test(
      '7. POST /OGAGLOW/orders/checkout/preview handles invalid coupon gracefully',
      () async {
        expect(sampleProductId, isNotNull);
        final payload = {
          'orderItems': [
    ...
    test(
      '8. POST /OGAGLOW/orders/placeOrder places a COD order successfully',
      () async {
        expect(sampleProductId, isNotNull);
        ...
        final res = await dio.post(
          '/OGAGLOW/orders/placeOrder',
          data: request.toJson(),
        );
        expect(res.statusCode == 200 || res.statusCode == 201, true);
        final placeOrderRes = PlaceOrderResponse.fromJson(
          res.data as Map<String, dynamic>,
        );
    ...
    test(
      '9. POST /auth/login returns structured 401 response for invalid credentials',
      () async {
        try {
          await dio.post(
            '/auth/login',
            data: {
              'email': 'non_existent_user@OGAGLOW.com',
